...
export const CheckoutLayout = () => {
  ...
  const planId = queryParams.get("plan")?.toLowerCase();
  const billingParam = queryParams.get("billing");

  useEffect(() => {
    // Only skip to step 2 if both plan and billing parameters are explicitly provided
  ...
  const plan = selectedPlan ? planDetails[planIdMap[selectedPlan] || selectedPlan] || planDetails.pro : planDetails.pro;
  ...
  const validateForm = (fields: (keyof FormData)[]) => {
    ...
    fields.forEach((field) => {
      ...
      else if (field === "zipCode" && !/^\d{5}(-\d{4})?$/.test(formData.zipCode))
  ...
  const nextStep = () => {
    const stepFields: (keyof FormData)[][] = [
      ...
      ["fullName", "email", "phone", "country", "location"], // Step 2: Personal info
