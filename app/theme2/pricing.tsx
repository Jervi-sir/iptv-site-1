...
const PricingPlans = ({ doRedirect = false }: { doRedirect?: boolean }) => {
  return (
    <section id="pricing" className="py-20 px-6 md:px-12 bg-zinc-950">
          ...
          {pricingPlans.map((plan) => (
            <div
              ...
              className={`rounded-xl flex flex-col overflow-hidden transition-all hover:shadow-xl hover:-translate-y-1 ${
                plan.isPopular
                  ? 'border-2 border-yellow-600 relative'
            ...
            >
              ...
              <div className="flex flex-col flex-1 p-6">
                ...
                {plan.savings && (
                  <div className="mb-4 text-yellow-600 font-semibold text-sm ">
                    {plan.savings}
                  </div>
                )}
                ...
                >
                  <Button
                    className={`w-full ${
                      plan.isPopular ? 'bg-yellow-600 text-zinc-950' : 'bg-zinc-950'
                    }`}
                  >
                    Choose {plan.title}
                  </Button>
